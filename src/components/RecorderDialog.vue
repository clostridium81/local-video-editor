...
async function startRecording() {
  ...
  try {
    if (mode.value === 'camera') {
      ...
      toast.warn('TTS モードは生成ボタンを使用してください')
      return
    }
    ...
    if (!stream) throw new Error('stream が取得できませんでした')
    ...
    console.error(err)
    toast.error('録画/録音の取得に失敗しました: ' + (err?.message ?? ''))
    stopStream()
  }
}
...
async function handleRecorderStop() {
  ...
  try {
    ...
    toast.success(`録画を素材として追加しました: ${name}`)
  } catch (err) {
    console.error(err)
    toast.error('素材への追加に失敗しました')
  }
  emit('close')
}
...
async function generateTTS() {
  ...
  if (!text) {
    toast.warn('テキストを入力してください')
    return
  }
  if (!window.speechSynthesis) {
    toast.error('この環境では TTS が使えません')
    return
  }
  ...
  try {
    ...
    await store.addAssetFromFile(file)
    toast.success('TTS 音声を素材として追加しました')
    emit('close')
  } catch (err: any) {
    console.error(err)
    toast.error('TTS 生成に失敗しました: ' + (err?.message ?? ''))
  }
}
  ...
  <div class="modal-backdrop" @click.self="emit('close')">
    <div class="modal">
      <div class="modal-head">
        <div class="title">録音・録画 / ナレーション</div>
        <button class="ghost close" :disabled="recording" @click="emit('close')">×</button>
      </div>
      <div class="modal-body">
        <div class="tabs">
          <button class="ghost" :class="{ active: mode === 'camera' }" @click="mode = 'camera'">カメラ</button>
          <button class="ghost" :class="{ active: mode === 'screen' }" @click="mode = 'screen'">画面</button>
          <button class="ghost" :class="{ active: mode === 'mic' }" @click="mode = 'mic'">マイク</button>
          <button class="ghost" :class="{ active: mode === 'tts' }" @click="mode = 'tts'">TTS</button>
        </div>

        <div v-if="mode !== 'tts'">
          <video ref="previewRef" class="preview" muted autoplay playsinline />
          <div class="state mono">{{ recording ? `REC ${fmt(elapsed)}` : '待機中' }}</div>
          <div class="actions">
            <button class="ghost" @click="emit('close')" :disabled="recording">閉じる</button>
            <button
              v-if="!recording"
              class="primary"
              @click="startRecording"
            >{{ mode === 'mic' ? '録音開始' : '録画開始' }}</button>
            <button v-else class="danger" @click="stopRecording">停止して素材に追加</button>
          </div>
        </div>
        ...
        <div v-else class="tts">
          ...
          <div class="row-2">
            ...
            <label class="field">
              <span>速度 <span class="mono">{{ ttsRate.toFixed(2) }}</span></span>
              <input type="range" min="0.5" max="2" step="0.1" v-model.number="ttsRate" />
            </label>
            <label class="field">
              <span>ピッチ <span class="mono">{{ ttsPitch.toFixed(2) }}</span></span>
          ...
          <div class="actions">
            <button class="ghost" @click="emit('close')">閉じる</button>
            <button class="primary" @click="generateTTS">TTS 生成</button>
          </div>
        </div>
      </div>
    </div>
